...
// PROFILE + LOCAL CLOCK
state.profile = state.profile || {name:"", bio:"Pelajar di Eduverse.id", avatar:"E"};
function saveState(){ localStorage.setItem(STORAGE_KEY, JSON.stringify(state)); }
function updateClock(){
  ...
  $("#clockZone").textContent=zone.replaceAll("_"," ");
  const pz=$("#profileZone"); if(pz) pz.textContent=zone.replaceAll("_"," ");
}
function renderProfile(){
  const p=state.profile || {name:"",bio:"Pelajar di Eduverse.id",avatar:"E"};
  const name=p.name || "Tamu Eduverse";
  $("#miniName").textContent=name.split(" ")[0];
  $("#miniAvatar").textContent=p.avatar || "E";
  $("#profileAvatarLarge").textContent=p.avatar || "E";
  $("#profileDisplayName").textContent=name;
  $("#profileBio").textContent=p.bio || "Pelajar di Eduverse.id";
  $("#profileXp").textContent=state.xp||0;
  $("#profileAttempts").textContent=state.attempts||0;
  $("#profileBest").textContent=`${state.best||0}%`;
  $("#profileNameInput").value=p.name||"";
  $("#profileBioInput").value=p.bio||"Pelajar di Eduverse.id";
  $("#profileAvatarSelect").value=p.avatar||"E";
}
function showWelcomeIfNeeded(){
  if(!state.profile || !state.profile.name){
    $("#profileWelcome").classList.remove("hidden");
    setTimeout(()=>$("#welcomeName").focus(),100);
  }
}
$("#saveProfile").addEventListener("click",()=>{
  const name=$("#profileNameInput").value.trim() || "Tamu Eduverse";
  state.profile={name,bio:$("#profileBioInput").value.trim()||"Pelajar di Eduverse.id",avatar:$("#profileAvatarSelect").value};
  saveState();renderProfile();
  addActivity("Profil diperbarui","Kartu profil Eduverse diperbarui");
});
$("#resetProfile").addEventListener("click",()=>{
  state.profile={name:"",bio:"Pelajar di Eduverse.id",avatar:"E"};saveState();renderProfile();showWelcomeIfNeeded();
});
$("#welcomeSave").addEventListener("click",()=>{
  const name=$("#welcomeName").value.trim();
  if(!name){alert("Masukkan nama terlebih dahulu.");return}
  state.profile={name,bio:"Pelajar di Eduverse.id",avatar:"E"};saveState();
  $("#profileWelcome").classList.add("hidden");renderProfile();addActivity("Masuk ke Eduverse",`Selamat datang, ${name}`);
});
$("#profileMini").addEventListener("click",()=>navTo("profile"));
updateClock();setInterval(updateClock,1000);renderProfile();showWelcomeIfNeeded();

// Real AI chat: calls the site's backend, never exposes the API key.
async function sendAIMessage(text=null){
  const input=$("#chatInput");
  const q=(text ?? input.value).trim();
  if(!q) return;
  appendChat("user",q); input.value=""; input.style.height="auto";
  $("#typing").classList.remove("hidden");
  const sendBtn=$("#sendChat"); sendBtn.disabled=true;
  try{
    const history=[...$("#chatMessages").querySelectorAll(".chat-msg")].slice(-12).map(el=>{
      const role=el.classList.contains("user")?"user":"assistant";
      return {role,content:el.querySelector(".chat-bubble").textContent};
    });
    ...
    if(!res.ok) throw new Error(data.error||"Server AI tidak tersedia");
